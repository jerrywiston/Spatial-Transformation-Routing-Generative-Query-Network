...
def eval(net, dataset, path, max_batch=1000, img_size=(64,64)):
    ...
    for it, batch in enumerate(data_loader):
        ...
        # Get Data
        x_obs = image[:,:3].reshape(-1,3,img_size[0],img_size[1]).to(device)
        v_obs = pose[:,:3].reshape(-1,7).to(device)
        ...
        x1_np = image[:,0].detach().permute(0,2,3,1).cpu().numpy()[0]
        x2_np = image[:,1].detach().permute(0,2,3,1).cpu().numpy()[0]
        x3_np = image[:,2].detach().permute(0,2,3,1).cpu().numpy()[0]
        x4_np = image[:,3].detach().permute(0,2,3,1).cpu().numpy()[0]
        img_draw = np.concatenate([x1_np, x2_np, x3_np, x4_np, x_query_sample], 1)*255
        img_draw = cv2.cvtColor(img_draw.astype(np.uint8), cv2.COLOR_BGR2RGB)
        img_draw[:,64-1:64+1,:] = 255
        img_draw[:,128-1:128+1,:] = 255
        img_draw[:,192-1:192+1,:] = 255
        img_draw[:,256-1:256+1,:] = 255
        cv2.imwrite(path+"result_"+str(i)+".jpg", img_draw) 
...
eval(net, test_dataset, obs_size=3, img_size=args.img_size)
