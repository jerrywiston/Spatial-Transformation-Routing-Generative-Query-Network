...
while(True):
    ...
    for it, batch in enumerate(train_loader):
        ...
        if steps % 100 == 0:
            ...
            print("[Ep %s (%s/%s)] loss_q: %f"%(str(epochs).zfill(4), str(steps), str(total_steps), loss_query))
        ...
        if steps % eval_step == 0:
            ...
            canvas = draw_result(net, train_dataset, obs_size, gen_size, (64,64))
            cv2.imwrite(fname, canvas)
            # Test
            fname = img_path+str(int(steps/eval_step)).zfill(4)+"_test.png"
            canvas = draw_result(net, test_dataset, obs_size, gen_size, (64,64))
            cv2.imwrite(fname, canvas)
            ...
            lh_train, _ = eval(net, train_dataset, obs_size=3, img_size=(64,64))
            print("Evaluate Testing Data ...")
            lh_test, _ = eval(net, test_dataset, obs_size=3, img_size=(64,64))
