...
class STRN(nn.Module):
    def __init__(self, n_view_cells, n_wrd_cells, vsize, wcode_size=3, emb_size=32, n_occ_layers=4, csize=128):
    ...
    def transform(self, v, inv=False, view_size=(16,16)):
        ...
        activation = torch.sigmoid(self.fa_net(wcode))
        occluding = torch.softmax(self.occ_net(wcode), 2)
    ...
    def forward(self, view_cell, v, view_size=(16,16)):
        ...
        return wrd_cell # (-1, csize, n_tgt_cells)
    ...
    def query(self, wrd_cell, v, view_size=(16,16), steps=None):
        ...
        # Initialize Canvas
        #query_view_cell = torch.zeros(wrd_cell.shape[0], self.tsize, view_size[0], view_size[1]).to(device)
        query_view_cell = wrd_cell * occ_mask[0].permute(0,2,1).repeat(1,self.csize,1)
        if steps is None:
            steps = self.n_occ_layers
        # Start Rendering
        for i in range(1,steps):
            mask_wrd_cell = wrd_cell * occ_mask[i].permute(0,2,1).repeat(1,self.csize,1)
            draw = torch.bmm(wrd_cell, route).reshape(-1, self.csize, view_size[0], view_size[1])
