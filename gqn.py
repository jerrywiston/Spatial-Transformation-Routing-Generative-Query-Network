...
class GQN(nn.Module):
    def __init__(self, ch=64, csize=256, vsize=7, draw_layers=6, down_size=4, share_core=False):
        ...
        self.encoder = Tower().to(device)
        self.generator = generator.GeneratorNetwork(x_dim=3, r_dim=csize+7, L=draw_layers, scale=down_size, share=share_core).to(device)

    def step_scene_fusion(self, x, v, n_obs):
        feature = self.encoder(x, v)
        scene_rep = torch.sum(feature.view(-1, n_obs, self.csize, 16, 16), 1, keepdim=False)
        return scene_rep
    ...
    def step_query_view_sample(self, scene_rep, vq):
        ...
        pose_code = pose_code.repeat(1, 1, 16, 16)
        scene_rep_pose = torch.cat((scene_rep, pose_code), dim=1) 
        x_query = self.generator.sample((64,64), scene_rep_pose)
        return x_query
